...
<script lang="ts">
  ...
  import {getBpmFormSchema,sysUsersColumns} from '../SysRoles.data';
  import {saveOrUpdate,sysUsersList} from '../SysRoles.api';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const refKeys = ref(['sysUsers', ]);
      const activeKey = ref('sysUsers');
      const sysUsers = ref();
      const tableRefs = {sysUsers, };
      ...
      const queryByIdUrl = '/test4/sysRoles/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(sysUsersList, {id: data.id}, sysUsersTable, ()=>{
          sysUsersTable.show = true;
        });
        //默认是禁用
